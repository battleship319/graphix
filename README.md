...
*   **Automated Patch Generation**: Integrates with LLMs (Gemini, OpenAI) to generate code patches for identified vulnerabilities.
...
You can optionally create a `.env` file in the root directory to store your API keys:
...
**Important**: The `knowledge_base/config.py` file is required to run this project but is not included in the repository (it contains sensitive credentials and is git-ignored for security).

**You must create this file manually before running the project.**
...
2.  Copy and paste the following template:
    ...
    WEAVIATE_COLLECTION_NAME = "CodeKnowledge" # Weaviate collection name (conventionally PascalCase)
...
**Security Note**: Never commit `config.py` to version control as it contains sensitive credentials. The file is already listed in `.gitignore`.
...
├── data/               # Database storage (Neo4j, Weaviate, Chroma)
